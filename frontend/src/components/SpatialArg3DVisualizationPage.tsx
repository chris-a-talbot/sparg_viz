...
export default function SpatialArg3DVisualizationPage() {
    ...
    const handleDownloadImage = async () => {
        ...
        try {
            ...
            const deckCanvas = containerRef.current.querySelector('canvas');
            if (!deckCanvas) {
                console.error('Could not find deck.gl canvas');
                return;
            }

            // Create a new canvas with the same dimensions
            const canvas = document.createElement('canvas');
            canvas.width = deckCanvas.width;
            canvas.height = deckCanvas.height;
            const ctx = canvas.getContext('2d');
            if (!ctx) return;

            // Fill with background color (dark blue)
            ctx.fillStyle = '#03303E';
            ctx.fillRect(0, 0, canvas.width, canvas.height);

            // Draw the deck.gl canvas onto our new canvas
            ...
            console.error('Error downloading 3D image:', error);
        }
    };
                ...
                <div className="max-w-7xl mx-auto flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-4">
                        <h1 className="text-2xl font-bold">
                            sp<span className="text-sp-pale-green">ARG</span>viz - 3D Spatial ARG
                        </h1>
